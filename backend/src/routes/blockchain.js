...
// POST /api/blockchain/digital-id
router.post('/digital-id', auth, (req, res) => {
  const user = db.users.find((u) => u.id === req.user.id);
  if (!user) return res.status(404).json({ error: 'User not found' });

  const existing = db.blockchainLogs.find(
    (b) => b.userId === req.user.id && b.type === 'digital_id'
  );
  if (existing) {
    return res.json({ digitalId: existing });
  }

  const previousHash = db.blockchainLogs.length > 0
...
router.get('/logs', auth, (req, res) => {
  const logs = req.user.role === 'admin'
    ? db.blockchainLogs
    : db.blockchainLogs.filter((b) => b.userId === req.user.id);
